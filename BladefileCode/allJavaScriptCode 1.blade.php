...
<div class="row row-sm">
    <div class="col-3">
        <div class="form-group">
            <label class="font-weight-600">Approved Conveyance (Rs). <span class="required-star">*</span></label>
            <input type="number" onchange="calcc();" id="Approved" name="Approved_Conveyance"
                ...
                placeholder="Enter Approved Conveyance (Rs)" data-local-required="true">
        </div>
    </div>
    <div class="col-3">
        <div class="form-group">
            <label class="font-weight-600">Bill Paid By. <span class="required-star">*</span></label>
            <select class="form-control" name="billpaid" id="billpaid" data-local-required="true">
    ...
    <script>
        function updateLocalRequiredFields() {
            var localSection = document.getElementById('local');
            var isLocalVisible = localSection && window.getComputedStyle(localSection).display !== 'none';

            conveyanceForm.querySelectorAll('[data-local-required="true"]').forEach(function(field) {
                field.required = isLocalVisible;
            });
        }

        updateLocalRequiredFields();

        ['conveyance', 'conveyancenongenerateconveyance', 'assignmentgenerate_id'].forEach(function(fieldId) {
            var field = document.getElementById(fieldId);

            if (field) {
                field.addEventListener('change', function() {
                    setTimeout(updateLocalRequiredFields, 0);
                });
            }
        });
    </script>
