...
<div class="row row-sm">
    ...
    <div class="form-group">
        ...
        <input type="text" name="name_as_per_bankaccount" id="name_as_per_bankaccount" class="form-control" value="{{ $neft->name_as_per_bankaccount ??'' }}"
...
<div class="col-4" id="payment" style="display: none">
    <div class="form-group">
        <label class="font-weight-600">Conveyance </label>
            ...
            <select class="form-control basic-multiple" multiple="multiple" name="localconveyance_id[]" id="conveyance_id"
               > 
            </select>
    </div>
</div>	
<div class="col-4">
    <div class="form-group">
    <label class="font-weight-600">Total Conveyacne</label>
    <input type="text" readonly name="totalconveyance" id="getconveyancesnefttotal" class="form-control" value="{{ $neft->nameofbank??'' }}"
        placeholder="Enter Total Conveyance">  
    </div>
</div>
</div>
<div class="form-group">
...
<script>
    $(function(){
       ...
       // alert(category_id);
        $.ajax({
            type: "get",
            url: "{{ url('getconveyancesneft') }}",
            data: "cid="+cid,
            success : function(res){
              
                $('#conveyance_id').html(res);
                debugger;
            },
            error : function(){
            },
        ...
        $('#employee').on('change', function () {
            ...
            $.ajax({
                ...
                //  debugger;
                    ...
                    $("#name_as_per_bankaccount").val(response.nameasperbank);
...
<script>
    $(function () {
        $('#conveyance_id').on('change', function () {
            var employee_id = $(this).val();

            $.ajax({
                type: "GET",
                url: "{{ url('getconveyancesnefttotal') }}",
                data: "employee_id=" + employee_id,
                success: function (response) {
                 // debugger;
                    $("#getconveyancesnefttotal").val(response);
