        ...
        <ol class="breadcrumb d-inline-flex font-weight-600 fs-13 bg-white mb-0 float-sm-right">
			 <li class="breadcrumb-item"> <div class="btn-group mb-2 mr-1">
                ...
                <div class="dropdown-menu">
                    @foreach($neftDateunique as $informationn => $neftDatass)
                    <a style="color: #37A000" class="dropdown-item"
                        href="{{url('/neftdate?'.'date='.$neftDatass->created_at )}}">{{ date('F d,Y', strtotime($neftDatass->created_at)) ??''}} {{ date('h:i A', strtotime($neftDatass->created_at)) }}</a>
                  @endforeach
                </div>
            </div></li>
           <li class="breadcrumb-item"><a class="btn btn-info-soft btn-sm" href="{{ url('neft')}}" style="color:#17a2b8"  >Back</a></li>
        </ol>
    </nav>
