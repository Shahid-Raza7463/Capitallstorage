                          ...
                          <div class="table-responsive">
                              ...
                              <div class="card-head" style="width:830px;height: 10px;">
                                  ...
                                  <b style="margin-left: 10px;float:left;margin-top: -17px;"> <a data-toggle="modal"
                                    data-target=".exampleModal1556-modal-lg" class="btn btn-info-soft btn-sm">Upload Confirmation File <i
                             ...
                             <table id="examplee" class="display nowrap">
                                  <thead>
                                      <tr>
										  ...
										  <th class="text-right"> <input type="checkbox" class="debtor-status-toggle-master" id="toggle-all-statuses"
                                                   data-toggle="toggle" data-style="ios" data-on=" Show" data-off=" Hide"></th>
                                          <th>Year</th>
                                          <th>Date</th>
                                          <th>Address</th>
                                          <th>Email</th>
                                      ...
                                      @foreach ($clientdebit as $clientdebitdata)
                                          <tr>
											   <td style="display: none;">{{$clientdebitdata->id }}</td>
                                                ...
                                                <td>   @if($clientdebitdata->status == 2)
											  ...
											  <td class="text-right"> 
												  ...
												  @endif
												        ...
												        @if($clientdebitdata->status == 2)
                                                <input type="checkbox" class="debtor-status-toggle" id="toggle-status-{{ $clientdebitdata->id }}" {{ $clientdebitdata->amounthidestatus ? 'checked' : '' }} data-toggle="toggle" data-style="ios" data-id="{{ $clientdebitdata->id }}" data-on=" Show" data-off=" Hide">
                                        @endif</td>
                                              <td>{{ $clientdebitdata->year }}</td>
											  @php
    $date = DateTime::createFromFormat('d/m/Y', $clientdebitdata->date);
    if ($date) {
                            ...
                            <table id="exampleee" class="display nowrap">
                                  <thead>
                                      <tr>
										  ...
										  <th class="text-right">  <input type="checkbox" class="creditor-status-toggle-master" id="toggle-all-statuses"
                                            data-toggle="toggle" data-style="ios" data-on=" Show" data-off=" Hide"></th>
                                          <th>Year</th>
                                          <th>Date</th>
                                          <th>Address</th>
                                      ...
                                      @foreach ($clientcredit as $clientcreditdata)
                                          <tr>
											    <td style="display: none;">{{$clientcreditdata->id }}</td>
                                                  ...
                                                  <td>   @if($clientcreditdata->status == 2) <input style="font-size:4px;" type="checkbox" name="approves[]"
											  ...
											  <td class="text-right">  	  @if($clientcreditdata->status != 2)
                                                ...
                                                <input type="checkbox" class="creditor-status-toggle" id="toggle-status-{{ $clientcreditdata->id }}" {{ $clientcreditdata->amounthidestatus ? 'checked' : '' }} data-toggle="toggle" data-style="ios" data-id="{{ $clientcreditdata->id }}" data-on=" Show" data-off=" Hide">
                                            @endif </td>
                                              <td>{{ $clientcreditdata->year }}</td>
											    @php
    $date = DateTime::createFromFormat('d/m/Y', $clientcreditdata->date);
    if ($date) {
        $formattedDate = $date->format('F d, Y');
    } else {
        $formattedDate = 'Invalid date';
                                  ...
                                  <thead>
                                      <tr>
										 <th style="display: none;">id</th>
										  ...
										  <th class="text-right">   <input type="checkbox" class="bank-status-toggle-master" id="toggle-all-statuses"
                                            data-toggle="toggle" data-style="ios" data-on=" Show" data-off=" Hide"></th>
                                          <th>Year</th>
										  <th>Date</th>
                                          <th>Address</th>
                                      ...
                                      @foreach ($clientbank as $clientbankdata)
                                          ...
                                          <tr>
											   <td style="display: none;">{{$clientbankdata->id }}</td>
                                                  ...
                                                  <td>  @if($clientbankdata->status == 2) <input style="font-size:4px;" type="checkbox" name="approvess[]"
                                                    value="{{ $clientbankdata->id }}">
@else
                                                    @endif
                                                </td>
											  ...
											  <td class="text-right"> 	    @if($clientbankdata->status != 2)
                                                ...
                                                <input type="checkbox" class="bank-status-toggle" id="toggle-status-{{ $clientbankdata->id }}" {{ $clientbankdata->amounthidestatus ? 'checked' : '' }} data-toggle="toggle" data-style="ios" data-id="{{ $clientbankdata->id }}" data-on=" Show" data-off=" Hide">
                                            @endif
												  </td>
                                              <td>{{ $clientbankdata->year }}</td>
											    @php
    $date = DateTime::createFromFormat('d/m/Y', $clientbankdata->date);
    if ($date) {
        $formattedDate = $date->format('F d, Y');
    } else {
        $formattedDate = 'Invalid date';
              ...
              <div class="modal-content">
                  <form id="detailsForm" method="post" action="{{ url('assignmentconfirmation/mail') }}"
                      ...
                      <div class="modal-header">
                          ...
                          <button id="refreshButtons" type="button" class="close" data-dismiss="modal" aria-label="Close">
                              <span aria-hidden="true">&times;</span>
                          </button>
                      </div>
                      <div class="modal-body">
                          ...
                          <div class="row row-sm">
                              ...
                              <div class="col-sm-8">
                                  ...
                                  <input placeholder=" Enter Subject" class="form-control" name="subject"
                                      type="text">
                                  <input hidden class="form-control" id="confirmationtype"  name="type" type="text">
                                  <input hidden class="form-control" id="debtorid" name="confirmationid[]" type="text">
                              ...
                              <div class="col-sm-4">
                                ...
                                <select required  id="assignmenttemplate" name="templateid" class="form-control">
                                  ...
                                  <input id="name" hidden class="form-control" name="assignmentgenerate_id" type="text"
                        ...
                        <input id="contactemail" placeholder=" Enter From Email" class="form-control" name="fromemail"
                            type="text">
                              ...
                              <div class="col-sm-6">
                                ...
                                <input  class="form-control" name="ccmail"
                                    type="text"  placeholder="Enter Multiple Mail Id">
                  ...
                  <form id="detailsForm" method="post" action="{{ url('confirmation/excel') }}"
                      ...
                      <div class="modal-header" style="background: #37A000">
                        ...
                        <!--  <div>
                          ...
                          <button id="refreshButtonexcel" type="button" class="close" data-dismiss="modal" aria-label="Close">
                              <span aria-hidden="true">&times;</span>
                          </button>
                      </div>
                      <div class="modal-body">
                          ...
                          <div class="details-form-field form-group row">
                              ...
                              <div class="col-sm-9">
                                  <input required class="form-control" id="input-excel" name="file" type="file"   onchange="validateFileType(this)">
                                  <input id="name" hidden class="form-control" name="assignmentgenerate_id" type="text"
                                      value="{{ $assignmentgenerate_id }}">
                              </div>
                          ...
                          <div class="details-form-field form-group row">
                              ...
                              <div class="col-sm-9">
                                  <a href="{{ url('backEnd/confirmationsfiles.xlsx') }}" class="btn btn-success btn">Download<i
                          ...
                          <div class="container mt-5">
                            ...
                            <nav aria-label="Page navigation example">
                                <ul class="pagination" id="pagination"></ul>
                                       ...
                                       <div class="modal-footer">
                          <button id="refreshButtonexcels" type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
...
function handleFileSelect(event) {
    ...
    reader.onload = function(e) {
        ...
        excelData = XLSX.utils.sheet_to_json(sheet, {header: 1, raw: false, dateNF: 'yyyy-mm-dd'});
    ...
    $(document).ready(function() {
        $('#examplee').DataTable({
		  "pageLength": 50,
            ...
            lengthMenu: [ [10, 30, 50, 100, -1], [10, 30, 50, 100, "All"] ], // Length dropdown options
            "order": [[ 0, "desc" ]],
			   columnDefs: [
                { orderable: false, targets: [1,5] } // Disables sorting on the first column (checkbox column)
            ],
            buttons: [
                {
                    extend: 'copyHtml5',
